...
int nth_best(int total_words, float scores[], int place) 
{
    /*
    ...
    */
    int counter = 0;
    int max = 0;
    float temp_scores[total_words];

    for(int w=1; w<total_words; w++) 
    {
...
void print_to_debug_file(int round, string guess_word, string hidden_word, string pos, string in_word, string pos_complement, string best)
{
    /*
...
string position_remover(string guess_word, string candidate, string temp) 
{
    /*
    ...
    */
    for (int l=0; l<6; l++) 
    {
        if (candidate[l] == guess_word[l] && candidate[l] != temp[l]) 
        {
            candidate = "";
        }
        else
...
string eliminated_letter_remover(string temp, string temp_comp, string guess_word) 
{
    /*
    ...
    */
    ...
    for (int l=0; l<5; l++) for (int ll=0; ll<temp_comp.length(); ll++)
    {
        if (guess_word[l] == temp_comp[ll] && temp[l] != temp_comp[ll]) 
        {
            guess_word = "";
        }
        else 
...
string make_position_guesses(int w, string word_array[], string temp)
{
    /*
    ...
    */
    ...
    {
        int count = 0;
        if (temp[l] != zero[0] && temp[l] == word_array[w][l] && count == 0)
        {
            position_guess = word_array[w];
            count++;
        }

        else if (temp[l] != zero[0] && temp[l] != word_array[w][l]) 
        {
            position_guess = "";
            break;
        }
